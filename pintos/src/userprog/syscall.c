...
struct vm_entry * addr_validation(void *addr, void *esp){
    void* temp_addr = addr;
...
void check_valid_string(const void *str, void *esp){
    ...
    struct vm_entry *vme = addr_validation((void*)str, esp);
    int tempsize = 0;
    if(vme ==NULL){
        exit(-1);
    }
...
void sendsig(pid_t pid, int signum)
{
   ...
   if( t->sig[signum].num == signum){
        printf("Signum: %d, Action: 0x%p\n",signum,t->sig[signum].SignalHandler);
   }
...
int mmap(int fd, void *addr){
    ...
    if((found_vme = addr_validation2(addr))!=NULL){
        return -1;
    }
